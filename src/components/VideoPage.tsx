...
import { ImageProps, VideoProps } from "@/utils/types"
...
const findImageByVideoId = (videoId: string, images: ImageProps[]): ImageProps => {
  const matching = images.find(image => image.videoId === videoId);
  return matching ? matching : images[0]
};


export default function VideoPage({ jibaroName, videos, images }: VideoPageProps) {
    // Create a state object that maps video IDs to their loading states
    const [loadingStates, setLoadingStates] = useState<{ [videoId: string]: boolean }>(
      videos.reduce((acc, video) => ({ ...acc, [video.videoId]: true }), {})
    );
